...
class Handler:
    ...
    def draw(self, window):
        ...
        # pygame.draw.polygon(window, (200,200,200), self.coords, 3)
...
while 1:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 3:
                ...
                distances =[]
                for dot in dots:
                    distances.append((dot.pos-pos).length())
                

                dots.insert(distances.index(min(distances))+1, Dot(vec2(*event.pos)))
                print(distances.index(min(distances)), len(dots))
    ...
    dofT.update(dt)
    factorT.update(dt)
    # h.dof += int(dofT.percentCompleted()*10)
    h.factor = factorT.percentCompleted()*6 + 1/2
    if h.factor >= 1:
        h.factor -= 1
